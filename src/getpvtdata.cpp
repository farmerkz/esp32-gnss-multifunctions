...
// Callback: выставляем флаги валидности даты и позиции для WiFi, при валидности
// позиции пишем в очередь данные для WiFi
// При валидности времени формируем дату и время для различных применений.
// Выставляем флаг валидности для трека
// Если не установлен флаг системного времени - выставляем системное время
